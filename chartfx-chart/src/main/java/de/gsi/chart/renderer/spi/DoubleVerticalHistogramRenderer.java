...
public class DoubleVerticalHistogramRenderer extends
    AbstractDataSetManagement<DoubleVerticalHistogramRenderer> implements Renderer {

    private int maxPoints;
    private boolean isSingleMode;

    public DoubleVerticalHistogramRenderer() {
        maxPoints = 300;
    }

    public DoubleVerticalHistogramRenderer(final int maxPoints) {
    ...
    @Override
    protected DoubleVerticalHistogramRenderer getThis() {
        return this;
    }

    /** If single mode, will be shown only one histogram if value is positive will be shown as positive histogram,
     * if value is negative will be shown negative histogram
     * */
                ...
                if (dataset.getDataCount() > 0) {
                    ...
                    if (true) { // d <= 1
                        ...
                        for (; i < Math.min(dataset.getXIndex(xmax) + 1, dataset.getDataCount()); i++) {
                            ...
                            double up = dataset.get(DoubleHistogramDataSet.DIM_X_LEFT, i);
                            double down = dataset.get(DoubleHistogramDataSet.DIM_X_RIGHT, i);

                            double xPos = xAxis.getDisplayPosition(x);
                            double yPos = yAxis.getDisplayPosition(0);
                            double upPos = yAxis.getDisplayPosition(up);
                            double downPos = yAxis.getDisplayPosition(down);

                            double yDiff = Math.abs(upPos - yPos);

                            if (isSingleMode) {
                                gc.setFill(Color.BLACK);
                                gc.fillRect(xPos - localBarWidth / 2, yPos + (up > 0 ? - yDiff : 0), localBarWidth, yDiff);
                            } else {
                                gc.setFill(Color.GREEN);
                                gc.fillRect(xPos - localBarWidth / 2, yPos - yDiff, localBarWidth, yDiff);

                                yDiff = Math.abs(downPos - yPos);
                                gc.setFill(Color.RED);
                                gc.fillRect(xPos - localBarWidth / 2, yPos, localBarWidth, yDiff);
                            }
                        }
                    }
                }
